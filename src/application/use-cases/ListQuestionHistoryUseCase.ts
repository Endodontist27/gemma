...
export class ListQuestionHistoryUseCase {
  ...
  async execute(sessionId: string): Promise<QuestionHistoryItemDto[]> {
    const questions = (await this.questionRepository.listBySession(sessionId)).filter(
      (question) => question.origin === 'user_local',
    );
      ...
      return [
        {
          question,
          answer,
          sources: sources.filter((source) => source.answerId === answer.id),
